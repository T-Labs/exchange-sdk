...
namespace TLabs.ExchangeSdk.Trading
{
    public interface IClientMarketdata
    {
        ...
        Task CreateFakeOrders(IReadOnlyList<Order> orders);
    ...
    public class ClientMarketdata : IClientMarketdata
    {
        ...
        {
            var result = await $"marketdata/deals/total-daily-volume-usdt".InternalApi()
        ...
        public async virtual Task CreateFakeOrders(IReadOnlyList<Order> orders)
        {
            await $"marketdata/orders/fake-orders".InternalApi()
                .PostJsonAsync(orders);
        }
